...
def check_sensorinf_in_db(sensor_id, rw_usr, rw_pass, db_loc):
    ...
    cursor_inf = list(cursor.observatories.find({"_id": sensor_id}))
    print(cursor_inf)
    if cursor_inf:
...
def test_insert_rawdat(sensor_id, sensor_script, rw_usr, rw_pass, db_loc):
    ...
    raw_test_dat = list(cursor.sensorDat.find({"observatories_id": sensor_id, "name": "test"}))
    if raw_test_dat:
...
def test_insert_magdat(sensor_id, sensor_script, rw_usr, rw_pass, db_loc):
    ...
    try:
        ...
        print("Failed to insert magnetometer data into database using script.")
        print(e)
        test_fail = 1
        
    raw_test_dat = list(cursor.magnetDat.find({"observatories_id": sensor_id, "name": "test"}))
    if raw_test_dat:
        print("Checking magnetometer data in db Success!")
    else:
        print("Checking magnetometer data in db Failed!")
        test_fail = 1
